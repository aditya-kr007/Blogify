...
const Signup=({type})=> {
    ...
    const {loading,Signup,data}=useSignup()
            ...
            <h1 style={{fontFamily:"serif"}} className='text-4xl capitalize text-center mb-20'>
                ...
                <div className='relative w-[100%] mb-6'>
                    <label className="input input-bordered flex items-center gap-2 rounded-full py-8 px-6">
                        ...
                        <input value={input.name} onChange={(e) => setInput({ ...input, name: e.target.value })} style={{ fontSize: "16px" }} type="text" name="name" placeholder="Name" id="name" className="grow" />
                    </label>
                </div>
                <div className='relative w-[100%] mb-6'>
                    <label className="input input-bordered flex items-center gap-2 rounded-full py-8 px-6">
                ...
                <div className='relative w-full flex items-center gap-2 my-8 opacity-10 uppercase font-bold'>
                    <hr className='w-1/2 border-white'></hr>
                    <p className='text-white'>or</p>
                    <hr className='w-1/2 border-white'></hr>
                </div>
                <button onClick={handleGoogleAuth} className='btn-dark flex items-center border-b  border-dark-grey justify-center gap-4 md-max:w-[100%] center'>
                    <img src={googleIcon} className='w-6' />
                    Continue With Google
                </button>
                <Link to={'/signin'}>
                    <p className='text-center pt-5 text-xl text-white'>Already have an account? <span className='font-medium text-2xl'>Sign In</span></p>
                </Link>
            </form>
        </section>
