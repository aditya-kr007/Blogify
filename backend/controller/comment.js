...
const commentOnBlog=(req,res)=>{
    ...
    new Comment(commentObj).save().then(async commentFile=>{
        ...
        new Notification(notificationObj).save().then(notify=>{
            console.log("new notification created")
        })
        return res.status(200).json({
            comment,commentedAt,children,_id:commentFile.id,user_id
        })
    })
}
...
const deleteComment=(_id)=>{
   ...
   .then(comment=>{
    ...
    .then(notification=>{
        // console.log("comment notification deleted deleted")
    })
...
const deleteComments=(req,res)=>{
    ...
    .then(comment=>{
        ...
        if(user_id==comment.commented_by || user_id==comment.blog_author){
            ...
            return res.status(403).json({error:"You cant't delete this comment"})
        }
    })
}
module.exports={commentOnBlog,getComment,getReplies,deleteComments}